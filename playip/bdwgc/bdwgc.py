...
def getWDB():
    global gwdb
    if not gwdb:
        gwdb = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER='+settings.server+';DATABASE='+settings.database+';UID='+settings.username+';PWD='+ settings.password)
    return gwdb
